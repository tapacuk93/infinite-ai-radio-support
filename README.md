...
Type a topic into the tune field and tap Tune (or press Return). The broadcast begins immediately and keeps going deeper the longer you listen — history, an idea, a city, anything.

**Do I need my own API key?**
Yes. In Settings, pick which AI narrates the broadcast — [Claude (Anthropic)](https://console.anthropic.com/settings/keys), [OpenAI](https://platform.openai.com/api-keys), [Gemini (Google)](https://aistudio.google.com/apikey), [Grok (xAI)](https://console.x.ai), or [DeepSeek](https://platform.deepseek.com/api_keys) — and enter that provider's own API key. Usage is billed directly by that provider to your own account — the app itself doesn't charge for generation. Your key is stored only on your device and is sent only to that provider's own API, never to any other server. See the [Privacy Policy](PRIVACY.md) for the full list of providers and their privacy policies.

**Can I go back to an earlier part of the broadcast, or branch into a new topic?**
Yes. Use the back button to return to where you came from, or the dive button to branch deeper into a specific idea mentioned in the current segment. Each dive starts its own thread, and you can always retrace your steps back up.
...
That means a call to your chosen AI provider's API failed, most commonly because the key was revoked, your account with that provider has run out of credits, or you've hit a rate limit — not something the app itself can recover from. Check your key and account status directly with that provider, then tap Tune In to reconnect.
